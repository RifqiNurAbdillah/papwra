...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const jenisBukuOptions = document.getElementById('jenis-buku-options'); // tempat tombol Ebook/Fisik
    ...
    if (jenisBukuOptions) {
        jenisBukuOptions.querySelectorAll('.jenis-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                loadBooks(type); // kirim type ke fungsi loadBooks
            });
        });
    }
    ...
    kategoriBukuOptions.querySelectorAll('.kategori-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // Cek tombol jenis buku yang aktif (jika ada)
            let type = '';
            if (jenisBukuOptions) {
                const activeJenis = jenisBukuOptions.querySelector('.jenis-btn.active');
                if (activeJenis) type = activeJenis.dataset.type;
            }

            // Tandai tombol kategori aktif
            kategoriBukuOptions.querySelectorAll('.kategori-btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');

            loadBooks('', type, category);
        });
    });
}
...
async function loadBooks(searchQuery = '', type = '', category = '') {
    ...
    // Tetap kompatibel dengan searchQuery
    ...
    // Tambahan filter kategori & jenis (jika dipanggil dari tombol)
